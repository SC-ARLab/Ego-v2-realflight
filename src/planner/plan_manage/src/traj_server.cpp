...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber poly_traj_sub = nh.subscribe("planning/trajectory", 10, polyTrajCallback); // 订阅轨迹
  ros::Subscriber heartbeat_sub = nh.subscribe("heartbeat", 10, heartbeatCallback); // ros::Time::now()

  pos_cmd_pub = nh.advertise<quadrotor_msgs::PositionCommand>("/position_cmd", 50); // 发布控制指令

  ros::Timer cmd_timer = nh.createTimer(ros::Duration(0.01), cmdCallback); // 根据轨迹计算信息
