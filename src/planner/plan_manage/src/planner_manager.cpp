...
namespace ego_planner
{
      ...
      Eigen::Vector3d &local_target_vel, bool &touch_goal)
  {
    ...
    double t_step = planning_horizen / 20 / pp_.max_vel_; // 以最大速度飞行的T/20
  ...
  bool EGOPlannerManager::planGlobalTrajWaypoints(
      ...
      const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc) // 规划 当前位置->路径点 的全局轨迹
  {

    poly_traj::MinJerkOpt globalMJO; // 多项式轨迹opt类
    Eigen::Matrix<double, 3, 3> headState, tailState; // 起始{p,v,a}
    headState << start_pos, start_vel, start_acc;
    tailState << waypoints.back(), end_vel, end_acc;
    Eigen::MatrixXd innerPts;  // 中间点

    if (waypoints.size() > 1) // 包含多个路径点
    {
    ...
    globalMJO.reset(headState, tailState, waypoints.size()); // 重置轨迹类状态

    double des_vel = pp_.max_vel_ / 1.5; // 期望速度
    Eigen::VectorXd time_vec(waypoints.size()); // 时间向量，每段轨迹T

    for (int j = 0; j < 2; ++j)
    {
      for (size_t i = 0; i < waypoints.size(); ++i) // 分配初始时间ti
      {
