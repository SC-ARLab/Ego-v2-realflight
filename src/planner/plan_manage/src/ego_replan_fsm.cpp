...
namespace ego_planner
{
  ...
  {
    exec_state_ = FSM_EXEC_STATE::INIT; // 初始化drone状态
    have_target_ = false; // flag:目标点
    have_odom_ = false; // flag:odom
    have_recv_pre_agent_ = false; // flag:通信
    flag_escape_emergency_ = true; // flag:
    mandatory_stop_ = false; // flag:命令stop
    ...
    visualization_.reset(new PlanningVisualization(nh)); // 智能指针
    planner_manager_.reset(new EGOPlannerManager); 
    planner_manager_->initPlanModules(nh, visualization_); // 初始化规划模块

    have_trigger_ = !flag_realworld_experiment_; // 触发信号，仿真时默认为true
    no_replan_thresh_ = 0.5 * emergency_time_ * planner_manager_->pp_.max_vel_; // 上界

    /* callback 定时器 */
    exec_timer_ = nh.createTimer(ros::Duration(0.01), &EGOReplanFSM::execFSMCallback, this); // 状态机，T=0.01s
    safety_timer_ = nh.createTimer(ros::Duration(0.05), &EGOReplanFSM::checkCollisionCallback, this); // 安全检查，T=0.05s

    odom_sub_ = nh.subscribe("odom_world", 1, &EGOReplanFSM::odometryCallback, this); // 订阅vins的odom
    mandatory_stop_sub_ = nh.subscribe("mandatory_stop", 1, &EGOReplanFSM::mandatoryStopCallback, this); // 订阅命令停止

    /* 通信部分 */ 
    /* Use MINCO trajectory to minimize the message size in wireless communication */
    broadcast_ploytraj_pub_ = nh.advertise<traj_utils::MINCOTraj>("planning/broadcast_traj_send", 10); // 发布轨迹话题：planning/broadcast_traj_send
    broadcast_ploytraj_sub_ = nh.subscribe<traj_utils::MINCOTraj>("planning/broadcast_traj_recv", 100, // 订阅轨迹话题：planning/broadcast_traj_recv
                                                                  &EGOReplanFSM::RecvBroadcastMINCOTrajCallback,
                                                                  this,
                                                                  ros::TransportHints().tcpNoDelay()); 

    poly_traj_pub_ = nh.advertise<traj_utils::PolyTraj>("planning/trajectory", 10); // 创建发布轨迹的pub
    data_disp_pub_ = nh.advertise<traj_utils::DataDisp>("planning/data_display", 100); // 创建展示轨迹的pub
    heartbeat_pub_ = nh.advertise<std_msgs::Empty>("planning/heartbeat", 10); // 创建发布心跳的pub
    ground_height_pub_ = nh.advertise<std_msgs::Float64>("/ground_height_measurement", 10); // 创建发布无人机高度的pub

    if (target_type_ == TARGET_TYPE::MANUAL_TARGET) // 手动选点
    {
      // waypoint_sub_ = nh.subscribe("/goal", 1, &EGOReplanFSM::waypointCallback3d, this); // 使用rviz里面3D nav goal
      waypoint_sub_ = nh.subscribe("/move_base_simple/goal", 1, &EGOReplanFSM::waypointCallback2d, this); // 使用2D nav goal
    }
    else if (target_type_ == TARGET_TYPE::PRESET_TARGET) // 预设点
    {
      trigger_sub_ = nh.subscribe("/traj_start_trigger", 1, &EGOReplanFSM::triggerCallback, this); // 订阅触发信号
    ...
    {
      cout << "Wrong target_type_value! target_type_=" << target_type_ << endl;
    }
  }
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    {
      // drone 0 || drone > 1 并且接受到前一架无人机的信息
  ...
  bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj)
  {
    ...
    if (plan_success)
    {
      ...
      polyTraj2ROSMsg(poly_msg, MINCO_msg);
      poly_traj_pub_.publish(poly_msg);
      broadcast_ploytraj_pub_.publish(MINCO_msg); // 广播自己的轨迹
    }
  ...
  bool EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp) // 规划路径点
  {
    bool success = false; 
    std::vector<Eigen::Vector3d> one_pt_wps; // 3x1
    one_pt_wps.push_back(next_wp);
    success = planner_manager_->planGlobalTrajWaypoints(
        odom_pos_, odom_vel_, Eigen::Vector3d::Zero(),
        one_pt_wps, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero()); // 规划全局轨迹
  ...
  // 3D Nav Goal
  void EGOReplanFSM::waypointCallback3d(const quadrotor_msgs::GoalSetPtr &msg)
  {
  ...
  // 2D nav goal
  void EGOReplanFSM::waypointCallback2d(const geometry_msgs::PoseStampedPtr &msg)
  {
    if (msg->pose.position.z < -0.1)
      return;

    // cout << "Triggered!" << endl;
    ROS_INFO("Received goal: %f, %f, %f", msg->pose.position.x, msg->pose.position.y, 0.8);

    Eigen::Vector3d end_wp(msg->pose.position.x, msg->pose.position.y, 0.8); // 终点（x,y,0.8） 
  ...
  void EGOReplanFSM::readGivenWpsAndPlan() // 读取预设路径点，
  {
    if (waypoint_num_ <= 0) // 路径点数 <= 0
    {
      ROS_ERROR("Wrong waypoint_num_ = %d", waypoint_num_);
      return;
    }

    wps_.resize(waypoint_num_); // vector -> matrix(size = waypoint_num)
    for (int i = 0; i < waypoint_num_; i++)
    {
    ...
    {
      visualization_->displayGoalPoint(wps_[i], Eigen::Vector4d(0, 0.5, 0.5, 1), 0.3, i); // 可视化目标点
      ros::Duration(0.001).sleep();
    }

    // plan first global waypoint
    wpt_id_ = 0; 
    planNextWaypoint(wps_[wpt_id_]); // 规划第id个路径点
  }
